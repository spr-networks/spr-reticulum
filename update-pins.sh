...
# Python pins: re-resolve rns (+ all transitive deps) for the runtime
# environment and regenerate requirements.txt with sha256 hashes for every
# release file (verified against a real download). Export RNS_VERSION=x.y.z
# first to pin a specific version instead of the latest release on PyPI.
echo "Resolving Python pins (rns + transitive deps)..." >&2
RNS_VERSION="$(./scripts/update-python-pins.sh ${RNS_VERSION:-})"
