...
WORKDIR /code
COPY code/ /code/
RUN --mount=type=tmpfs,target=/root/go/ go build -trimpath -ldflags "-s -w" -o /reticulum_plugin /code/
...
ENV DEBIAN_FRONTEND=noninteractive
ARG UBUNTU_SNAPSHOT=20260601T000000Z
ARG RNS_VERSION=1.3.7
# Python 3 from the Ubuntu snapshot repo only (reproducible apt inputs).
RUN set -eux; \
    printf 'Types: deb\nURIs: https://snapshot.ubuntu.com/ubuntu/%s\nSuites: noble noble-updates noble-security\nComponents: main restricted universe multiverse\nSigned-By: /usr/share/keyrings/ubuntu-archive-keyring.gpg\n' "${UBUNTU_SNAPSHOT}" > /etc/apt/sources.list.d/ubuntu.sources; \
    printf 'APT::Install-Recommends "false";\nAcquire::Check-Valid-Until "false";\n' > /etc/apt/apt.conf.d/99reproducible; \
    rm -f /etc/apt/sources.list.d/ubuntu.list /etc/apt/sources.list; \
    apt-get update && apt-get install -y --no-install-recommends python3 python3-venv && \
    rm -rf /var/lib/apt/lists/* /var/log/* /var/cache/ldconfig/aux-cache
# rns + transitive deps: fully pinned, hash-checked wheels only (no sdist builds).
# requirements.txt is generated by scripts/update-python-pins.sh; --no-compile
# keeps timestamps out of the layer (no .pyc), PYTHONDONTWRITEBYTECODE keeps it so.
COPY requirements.txt /requirements.txt
ENV PYTHONDONTWRITEBYTECODE=1
RUN set -eux; \
    python3 -m venv /opt/rns; \
    /opt/rns/bin/pip install --no-cache-dir --no-compile --require-hashes --only-binary :all: -r /requirements.txt; \
    test "$(/opt/rns/bin/pip show rns | awk '/^Version:/{print $2}')" = "${RNS_VERSION}"; \
    ln -s /opt/rns/bin/rnsd /opt/rns/bin/rnstatus /opt/rns/bin/rnpath /usr/local/bin/; \
    rm -rf /root/.cache
COPY scripts /scripts/
COPY --from=builder /reticulum_plugin /
